package com.wallmart.routedemo.network;

import android.app.ProgressDialog;
import android.content.Context;

import com.wallmart.routedemo.BuildConfig;
...
public class NetworkCall implements INetwork {
    ...
    * */
    ...
    @Override
    public Subscription callServiceForRotes(final RoutesCallback callback, boolean showLoading) {
        if (showLoading) {
            showLoading();
        }
        return api.callServiceForRoutes()
                ...
                .onErrorResumeNext(new Func1<Throwable, Observable<? extends RoutesResponse<ArrayList<RoutesData>>>>() {
                    @Override
                    public Observable<? extends RoutesResponse<ArrayList<RoutesData>>> call(Throwable throwable) {
                        return Observable.error(throwable);
                    }
                })
                .subscribe(new Subscriber<RoutesResponse<ArrayList<RoutesData>>>() {
                    @Override
                    ...
                    @Override
                    public void onNext(RoutesResponse<ArrayList<RoutesData>> cityListResponse) {
